...
public final class SelectExtension<Item extends IItem<? extends RecyclerView.ViewHolder>> implements IAdapterExtension<Item> {
    ...
    public final Set<Integer> getSelections() {
        IntRange intRangeUntil = RangesKt.until(0, this.fastAdapter.getGlobalSize());
        ...
        while (it.hasNext()) {
            ...
            numValueOf.intValue();
            IItem item = this.fastAdapter.getItem(iNextInt);
            if (item == null || !item.getIsSelected()) {
                numValueOf = null;
            }
            if (numValueOf != null) {
                arraySet.add(numValueOf);
            }
        }
        return arraySet;
    }
    ...
    public final void select(final boolean considerSelectableFlag) {
        this.fastAdapter.recursive((AdapterPredicate) new AdapterPredicate<Item>() { // from class: com.mikepenz.fastadapter.select.SelectExtension.select.1
            @Override // com.mikepenz.fastadapter.utils.AdapterPredicate
    ...
    public final void deselect() {
        this.fastAdapter.recursive((AdapterPredicate) new AdapterPredicate<Item>() { // from class: com.mikepenz.fastadapter.select.SelectExtension.deselect.1
            @Override // com.mikepenz.fastadapter.utils.AdapterPredicate
            public boolean apply(IAdapter<Item> lastParentAdapter, int lastParentPosition, Item item, int position) {
                Intrinsics.checkParameterIsNotNull(lastParentAdapter, "lastParentAdapter");
                Intrinsics.checkParameterIsNotNull(item, "item");
    ...
    public final List<Item> deleteAllSelectedItems() {
        ...
        this.fastAdapter.recursive((AdapterPredicate) new AdapterPredicate<Item>() { // from class: com.mikepenz.fastadapter.select.SelectExtension.deleteAllSelectedItems.1
            @Override // com.mikepenz.fastadapter.utils.AdapterPredicate
            public boolean apply(IAdapter<Item> lastParentAdapter, int lastParentPosition, Item item, int position) {
                IParentItem<?> parent;
