...
/* JADX INFO: compiled from: IItemAdapter.kt */
...
public interface IItemAdapter<Model, Item extends IItem<? extends RecyclerView.ViewHolder>> extends IAdapter<Item> {
    IItemAdapter<Model, Item> add(int position, List<? extends Model> items);

    IItemAdapter<Model, Item> add(int position, Model... items);

    IItemAdapter<Model, Item> add(List<? extends Model> items);

    IItemAdapter<Model, Item> add(Model... items);

    IItemAdapter<Model, Item> addInternal(int position, List<? extends Item> items);

    IItemAdapter<Model, Item> addInternal(List<? extends Item> items);
    ...
    IItemAdapter<Model, Item> move(int fromPosition, int toPosition);
    ...
    IItemAdapter<Model, Item> set(List<? extends Model> items);

    void setIdDistributor(IIdDistributor<Item> iIdDistributor);

    IItemAdapter<Model, Item> setInternal(int position, Item item);

    IItemAdapter<Model, Item> setNewList(List<? extends Model> items, boolean retainFilter);

    /* JADX INFO: compiled from: IItemAdapter.kt */
    @Metadata(bv = {1, 0, 3}, k = 3, mv = {1, 1, 16})
    public static final class DefaultImpls {
        public static <Model, Item extends IItem<? extends RecyclerView.ViewHolder>> Item peekAdapterItem(IItemAdapter<Model, Item> iItemAdapter, int i) {
            return (Item) IAdapter.DefaultImpls.peekAdapterItem(iItemAdapter, i);
        }
