...
public abstract class Random {
        ...
        if (j2 > 0) {
            if (((-j2) & j2) == j2) {
                ...
                if (i != 0) {
                    ...
                    jNextBits = (((long) nextBits(RandomKt.fastLog2(i2))) << 32) + (((long) nextInt()) & ExpandableGridView.PACKED_POSITION_VALUE_NULL);
                }
                jNextBits = ((long) iNextInt) & ExpandableGridView.PACKED_POSITION_VALUE_NULL;
            } else {
                do {
    ...
    public byte[] nextBytes(byte[] array, int fromIndex, int toIndex) {
        ...
        if (!new IntRange(0, array.length).contains(fromIndex) || !new IntRange(0, array.length).contains(toIndex)) {
            throw new IllegalArgumentException(("fromIndex (" + fromIndex + ") or toIndex (" + toIndex + ") are out of range: 0.." + array.length + FileUtil.SUFFIX_SEPARATOR_CHAR).toString());
        }
