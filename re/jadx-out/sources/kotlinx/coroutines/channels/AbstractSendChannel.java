...
public abstract class AbstractSendChannel<E> implements SendChannel<E> {
    ...
    */
    ...
    public final Object mo1908trySendJP2dKIU(E element) {
        Object objOfferInternal = offerInternal(element);
        if (objOfferInternal == AbstractChannelKt.OFFER_SUCCESS) {
            return ChannelResult.INSTANCE.m1928successJP2dKIU(Unit.INSTANCE);
        }
        if (objOfferInternal == AbstractChannelKt.OFFER_FAILED) {
            Closed<?> closedForSend = getClosedForSend();
            return closedForSend == null ? ChannelResult.INSTANCE.m1927failurePtdJZtk() : ChannelResult.INSTANCE.m1926closedJP2dKIU(helpCloseAndGetSendException(closedForSend));
        }
        if (objOfferInternal instanceof Closed) {
            return ChannelResult.INSTANCE.m1926closedJP2dKIU(helpCloseAndGetSendException((Closed) objOfferInternal));
        }
    ...
    public final void helpCloseAndResumeWithSendException(Continuation<?> continuation, E e, Closed<?> closed) {
        ...
        if (function1 != null && (undeliveredElementExceptionCallUndeliveredElementCatchingException$default = OnUndeliveredElementKt.callUndeliveredElementCatchingException$default(function1, e, null, 2, null)) != null) {
            ...
            continuation.resumeWith(Result.m352constructorimpl(ResultKt.createFailure(undeliveredElementException)));
            return;
        }
    ...
    private final void helpClose(Closed<?> closed) {
        ...
        while (true) {
            ...
            if (receive == null) {
                break;
            } else if (!receive.mo1953remove()) {
                receive.helpRemove();
            } else {
                objM1945constructorimpl$default = InlineList.m1950plusFjFbRPM(objM1945constructorimpl$default, receive);
            }
        }
        if (objM1945constructorimpl$default != null) {
            if (!(objM1945constructorimpl$default instanceof ArrayList)) {
                ((Receive) objM1945constructorimpl$default).resumeReceiveClosed(closed);
            } else if (objM1945constructorimpl$default != null) {
                ArrayList arrayList = (ArrayList) objM1945constructorimpl$default;
