...
impl InputEmitter {
    ...
    pub fn process_pen(&mut self, payload: &[u8], pen_cursor: bool) -> Result<()> {
        if payload.len() < 11 {
            return Ok(());
        }
        ...
        let eraser = flags & PEN_FLAG_ERASER != 0;

        debug!(
            "Pen: {} {}{} at ({:.0}, {:.0}) pressure={:.3} orient={:.2} tilt={:.2} raw_tilt={} raw_orient={}",
            state,
            if eraser { "[ERASER] " } else { "" },
            if button { "[BTN]" } else { "" },
            x,
            y,
            pressure,
        ...
        {
            if let Some(inj) = &mut self.injector {
                // Convert spherical (tilt angle, azimuth) → Cartesian (tilt_x, tilt_y)
                // tilt=0 → perpendicular → no tilt. tilt=π/2 → flat → max tilt.
                // Use sin(tilt), NOT cos(tilt) — cos gives inverted values.
