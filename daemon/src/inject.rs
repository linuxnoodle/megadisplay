    ...
    impl PenInjector {
        pub fn new(width: u32, height: u32) -> Result<Self> {
            ...
            let dev = VirtualDevice::builder()?
                ...
                .with_absolute_axis(&UinputAbsSetup::new(
                    AbsoluteAxisCode::ABS_X,
                    AbsInfo::new(0, 0, width as i32, 0, 0, 10),
                ))?
                .with_absolute_axis(&UinputAbsSetup::new(
                    AbsoluteAxisCode::ABS_Y,
                    AbsInfo::new(0, 0, height as i32, 0, 0, 10),
                ))?
                .with_absolute_axis(&UinputAbsSetup::new(
        ...
        pub fn emit(
            ...
            // Tool change: release old tool first
            if self.current_tool != desired_tool {
                if self.current_tool == PenTool::Pen {
                    events.push(InputEvent::new_now(
                        EventType::KEY.0,
                        KeyCode::BTN_TOOL_PEN.0,
                        0,
                    ));
                } else if self.current_tool == PenTool::Rubber {
                    events.push(InputEvent::new_now(
                        EventType::KEY.0,
                ...
                if desired_tool == PenTool::Pen {
                    events.push(InputEvent::new_now(
                        EventType::KEY.0,
                        KeyCode::BTN_TOOL_PEN.0,
                        1,
                    ));
                } else if desired_tool == PenTool::Rubber {
                    events.push(InputEvent::new_now(
                        EventType::KEY.0,
                        KeyCode::BTN_TOOL_RUBBER.0,
                        1,
            ...
            } else if in_range != self.pen_in_range {
                // Just a range change without tool change
                if desired_tool == PenTool::Pen {
                    events.push(InputEvent::new_now(
                        EventType::KEY.0,
                        KeyCode::BTN_TOOL_PEN.0,
